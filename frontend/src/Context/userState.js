...
const UserState = (props) => {
  ...
  const loadRazorpay = () => {
    ...
    script.onload = async () => {
      try {
        ...
        } = await axios.get("http://localhost:5000/api/investor/get-razorpay-key", {
          headers: {
            "Content-Type": "application/json",
            "auth-token": localStorage.getItem("token"),
          },
  ...
  const getInvestmentData = async () => {
    const response = await axios
      .get("http://localhost:5000/api/investor/getTransactions", {
        headers: {
          "Content-Type": "application/json",
          "auth-token": localStorage.getItem("token"),
        },
  ...
  const getUserData = async () => {
    try {
      ...
      if (error.response && error.response.status === 401) {
        console.error("User authentication failed. Redirecting to login page.");
        // Redirect the user to the login page or show a message
      }
    }
  };
  ...
  const getStartups = async () => {
    const response = await axios.get(`http://localhost:5000/api/investor/fetch-startups`, {
      headers: {
        "Content-Type": "application/json",
        "auth-token": localStorage.getItem("token"),
      },
  ...
  const getUserStartups = async () => {
    try {
      ...
      if (!token) {
        console.error("No token found. User is not authenticated.");
        // Perform actions for unauthenticated user, such as redirecting to login page
        return;
      }
  
      const response = await axios.get(`http://localhost:5000/api/investor/fetchuserStartups`, {
        headers: {
          "Content-Type": "application/json",
          "auth-token": token,
        },
      ...
      if (response.data.success) {
        ...
        console.error("Failed to fetch user startups:", response.data.msg);
