...
const Form = () => {
    ...
    const createStartup = async (e) => {
        ...
        try {
            ...
            if (!token) {
                showAlert("Authentication token not found. Please login again.");
                return;
            }
    
            const response = await axios.post(`http://localhost:5000/api/investor/create-startup`, credentials, {
                headers: {
            ...
            if (response && response.data && response.data.success) {
                ...
                showAlert("Failed to create startup. Please try again later.");
            }
        } catch (error) {
